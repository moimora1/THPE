...
Versión 1.7 — correcciones respecto a v1.6:
...
def build_DESI_vector(use_covariance=True):
    """
    ...
    Devuelve:
        ...
        cov_inv : inversa
    """
    z_bao = np.array([d['z']   for d in DESI_DR1])
    kinds = [d['kind']         for d in DESI_DR1]
    y_obs = np.array([d['val'] for d in DESI_DR1])
    sig   = np.array([d['sig'] for d in DESI_DR1])

    n   = len(y_obs)
    cov = np.diag(sig**2)

    if use_covariance:
        for tracer, r in DESI_CORR.items():
            idx = [i for i, d in enumerate(DESI_DR1) if d['name'] == tracer]
            if len(idx) == 2:
                i, j = idx
                cov[i, j] = cov[j, i] = r * sig[i] * sig[j]
        print(f"  DESI DR1: {n} observables "
        ...
        print(f"  DESI DR1: {n} observables, covarianza diagonal (--no-cov)")
...
def run_MCMC(args_like, n_walkers=32, n_steps=5000, n_burn=1000, seed=42):
...
def print_results(samples):
    ...
    names = ['Φ₀', 'α', 'β', 'γ']
    print("\n" + "="*60)
    print("RESULTADOS DEL AJUSTE THPE v1.7")
    print("="*60)
...
def plot_results(samples, z_bao, kinds, y_obs, sig, z_sn, mu_obs, sig_mu,
    ...
    fname = os.path.join(outdir, 'THPE_v17_resultados.png')
    ...
    if HAS_CORNER:
        ...
        f2 = os.path.join(outdir, 'THPE_v17_corner.png')
...
def main():
    ...
    print("="*70)
    print("THPE v1.7 — Ajuste a DESI DR1 (BAO) + Pantheon+ (SNIa)")
    ...
    z_bao, kinds, y_obs, sig, cov_bao, cov_inv_bao = \
        build_DESI_vector(use_covariance=use_cov)
